...
function Write-TrainingBlock($ws,$ref,$title,$focus,$tips,$time,$acts){
    ...
    SetC $ws $ref 1 "$focus  |  $time  |  $tips" $false 10 $headerBlue $lightGray $true
    MC $ws $ref 1 8; $ref++
...
SetC $ws0 1 1 "王丽颖  减脂塑形方案 V2" $true 22 $gold $darkBg $true
MC $ws0 1 1 5
...
foreach($fw in $firstWeek){
    ...
    SetC $ws0 $row 3 $fw[2] $false 11 0 0 $true
    MC $ws0 $row 3 5
...
SetC $ws0 $row 5 "本周第一要务" $true 10 $white $headerBlue $false
$row++
$o4=@(
    @("1","唤醒","不训练","8000步/天","戒含糖饮料+3L水+睡够7h  三件做到两件=胜利"),
    @("2","入门","3次/周 固定器械","8000步+练后走10min","走进健身房就是胜利  怎么选重量看Sheet2"),
    @("3","适应","4次/周 上下肢分化","8000-10000步","三餐规律 每餐有蛋白质 晚餐控碳水"),
...
$idx=@(
    ...
    @("第2-4周每次练什么(含怎么选重量)"," Sheet 第2-4周训练"),
    @("热身怎么热身 拉伸怎么拉伸"," Sheet 热身有氧拉伸"),
    @("饮食怎么吃(含外食/外卖/PMS/经期)"," Sheet 饮食与规则"),
    @("状态差/来例假/太累了 怎么办"," Sheet 备用与复盘"),
    @("4周后怎么看效果 拍照怎么拍"," Sheet 备用与复盘")
)
foreach($ix in $idx){
    SetC $ws0 $row 1 $ix[0] $true 10 $headerBlue $lightGray $false
    SetC $ws0 $row 2 $ix[1] $false 10 0 0 $false
    MC $ws0 $row 2 5
    $ws0.Range("A$($row):E$($row)").RowHeight=22
    SB $ws0 $row 1 5; $row++
}

$ws0.Range("A:A").ColumnWidth=28
$ws0.Range("B:B").ColumnWidth=20
$ws0.Range("C:E").ColumnWidth=20
...
MC $ws 2 1 8
$ws.Range("A2:H2").Font.Color=$white; $ws.Range("A2:H2").Interior.Color=$darkBg
...
SetC $ws2 2 1 "第1周不训练（只做封面三件事）。第2周走进健身房。第3-4周开始规律训练。训练前先看下方[怎么选重量]。" $false 10 $headerBlue $lightGray $true
MC $ws2 2 1 8

# Weight selection guide (NEW V2)
$r=4
...
foreach($wg in $weightGuide){
    SetC $ws2 $r 1 $wg $false 10 0 0 $true
    MC $ws2 $r 1 8
...
SetC $ws2 $r 1 "第2周目标：走进健身房就是胜利。三个训练日做同样内容——先熟悉动作，不急着变换。全程固定器械，轨道固定，一个人安全。" $false 10 $headerBlue $lightGray $true
MC $ws2 $r 1 8
$ws2.Range("A$($r):H$($r)").RowHeight=22; $r++

$actsW2=@(
    @("坐姿推胸","3组","12-15次","先挂最轻片 按选重三步法调整","60秒","轻松-中等","【呼吸】推出呼气、收回吸气。【要点】后背贴紧靠背。推到前面不锁死手肘。先找到'胸肌在用力'的感觉。"),
...
$r=Write-TrainingBlock $ws2 $r "第2周训练（三个训练日都做这个）" "固定器械入门  找到发力感  |  35-40分钟" "动作质量优先——不要和别人比" "38-42" $actsW2
...
# Lower A
$actsLA=@(
    @("坐姿腿举","3组","10-12次","比第2周重1-2片(按三步法)","75秒","中等偏强","【呼吸】推出呼气、回收吸气。【发力感】脚后跟发力蹬，大腿前侧和臀部一起用力。"),
    @("坐姿髋外展","3组","15-20次","比第2周重1片","45秒","中等","【发力感】打开时臀外侧收紧保持1秒。改善胯宽和臀凹陷的关键动作。"),
    @("坐姿腿屈伸","3组","12-15次","轻-中重量","60秒","中等","【呼吸】踢出呼气、放下吸气。【要点】脚尖回勾，大腿前侧发力。膝盖不舒服的话降重量。"),
    @("哑铃臀桥","3组","12-15次","双手持一颗哑铃6-10kg放髋部","60秒","中等","【呼吸】推起呼气、下放吸气。【要点】下巴微收。顶峰夹紧屁股1-2秒。改善臀凹陷和妈妈臀。"),
...
SetC $ws3 1 1 "有氧安排  热身  拉伸" $true 16 $headerBlue 0 $true
...
# 8000步再贴一次拆解 (V2)
$r++
SetC $ws3 $r 1 "8000步再拆一次——贴在热身旁边方便看" $true 12 $headerBlue $lightBlue $true
MC $ws3 $r 1 8; $r++

$stepDetail=@(
    @("早上通勤多走一站","+1500步","早出门10分钟 提前一站下车"),
    @("午休溜达10分钟","+2000步","吃完午饭别马上坐"),
    @("下午接水/上厕所多走两趟","+1000步","绕远路 积少成多"),
    @("晚饭后散步20分钟","+2500步","最重要的一段——饭后别坐"),
    @("晚上在家走动/收拾","+1000步","做家务也算步数"),
    @("全天合计"," 8000步","走不够5000也比不走强")
)
SetC $ws3 $r 1 "场景" $true 10 $white $headerBlue $false
SetC $ws3 $r 2 "步数" $true 10 $white $headerBlue $false
SetC $ws3 $r 4 "怎么做" $true 10 $white $headerBlue $false
MC $ws3 $r 4 8; $r++
foreach($sd in $stepDetail){
    SetC $ws3 $r 1 $sd[0] $true 10 $headerBlue $lightGray $false
    SetC $ws3 $r 2 $sd[1] $false 10 0 0 $false
    SetC $ws3 $r 4 $sd[2] $false 10 0 0 $true
    MC $ws3 $r 4 8
...
SetC $ws3 $r 5 "要点" $true 9 $white $headerBlue $false; MC $ws3 $r 5 8
$r++
...
# Standard photo guide (V2 NEW)
$r+=2
SetC $ws3 $r 1 "  拍照标准（今天就拍——4周后同一位置同一光线对比）" $true 14 $white $accentGreen $true
MC $ws3 $r 1 8
$ws3.Range("A$($r):H$($r)").RowHeight=26
$ws3.Range("A$($r):H$($r)").Font.Color=$white
$ws3.Range("A$($r):H$($r)").Interior.Color=$accentGreen
...
SetC $ws3 $r 1 "今天拍一组。4周后同一时间、同一光线、同一套衣服再拍一组。不要吸肚子！不要找角度！真实的才有对比价值。" $false 10 $headerBlue $lightGray $true
MC $ws3 $r 1 8; $r++

SetC $ws3 $r 1 "拍什么" $true 10 $white $headerBlue $false
SetC $ws3 $r 2 "怎么拍" $true 10 $white $headerBlue $false
MC $ws3 $r 2 5
SetC $ws3 $r 6 "要点" $true 10 $white $headerBlue $false
$r++
...
$photoGuide=@(
    ...
    @("穿什么","运动内衣+紧身裤或短裤","露出腰腹 才能看到变化。不要穿宽松T恤拍。"),
...
$nutri=@(
    ...
    @("碳水","160-180g","米饭每餐半拳到一拳。优先杂粮饭/红薯/玉米。晚餐碳水比午餐少一半。水果一天1-2份(一根香蕉或一个苹果的量)。"),
    @("脂肪","40-50g","炒菜控油。不吃油炸。坚果一天一小把够了。"),
    @("饮水","3L/天","买1.5L大水壶 上午一壶下午一壶。水里加柠檬片。戒所有含糖饮料——这是你第一周掉秤的最大来源。"),
    @("蔬菜","不限量","每餐占一半盘子。先吃蔬菜再吃肉最后吃饭——这个顺序掉秤更快。")
)
foreach($n in $nutri){
...
$meals=@(
    ...
    @("午餐","米饭半拳+一份肉/鱼(手掌大)+蔬菜占半盘","食堂/外卖：选清蒸/炖/卤 避开油炸和勾芡。先吃菜再吃肉最后吃饭。"),
    @("下午加餐","小水果/无糖酸奶/坚果10颗以内","饿了吃 不饿可以不吃。不要饿到晚餐暴食。"),
    @("晚餐","米饭半拳或不吃+一份肉/鱼/豆腐+大量蔬菜","晚餐控碳水是掉秤关键。晚上饿吃黄瓜/番茄/喝温水。"),
    @("训练后","蛋白粉+水或牛奶","训练完30分钟内喝。如果不买蛋白粉——训练后半小时内吃含蛋白质的正餐。")
)
foreach($m in $meals){
...
# OUTSIDE FOOD GUIDE (V2 NEW)
$r+=2
SetC $ws4 $r 1 "如果今天吃外卖/外食——这样选  V2新增" $true 14 $white $darkBg $true
MC $ws4 $r 1 8
$ws4.Range("A$($r):H$($r)").RowHeight=26
$ws4.Range("A$($r):H$($r)").Font.Color=$white
...
# PMS SECTION (V2 NEW)
$r+=2
SetC $ws4 $r 1 "经期前一周（PMS阶段）——提前知道就不慌  V2新增" $true 14 $white $softPink $true
MC $ws4 $r 1 8
$ws4.Range("A$($r):H$($r)").RowHeight=26
$ws4.Range("A$($r):H$($r)").Font.Color=$white
...
$pms=@(
    ...
    @("训练时觉得比平时累","身体在准备来月经 能量储备偏低","降重量。RPE比平时低1-2分。不要求'突破'——能完成就是胜利。")
)
foreach($pm in $pms){
...
# Period & habits
$r++
SetC $ws4 $r 1 "来例假时 & 这4周的生活习惯重点" $true 14 $white $darkBg $true
MC $ws4 $r 1 8
$ws4.Range("A$($r):H$($r)").RowHeight=26
$ws4.Range("A$($r):H$($r)").Font.Color=$white
$ws4.Range("A$($r):H$($r)").Interior.Color=$darkBg
...
SetC $ws4 $r 1 "来例假了" $true 10 $accentRed $lightGray $false
SetC $ws4 $r 2 "第1-2天休息或只走路。第3天后如果OK恢复轻量训练。经期训练不勉强——身体在流血 需要休息不是偷懒。" $false 10 0 0 $true
MC $ws4 $r 2 8
$ws4.Range("A$($r):H$($r)").RowHeight=28; SB $ws4 $r 1 8; $r++

SetC $ws4 $r 1 "每周习惯" $true 10 $headerBlue $lightGray $false
SetC $ws4 $r 2 "第1周=戒含糖饮料 第2周=三餐规律 第3周=先吃菜再吃肉最后吃饭 第4周=复盘  最难坚持的餐是哪顿？最好的习惯是什么？" $false 10 0 0 $true
MC $ws4 $r 2 8
...
foreach($r in $rules){
    SetC $ws4 $r 1 $r[0] $true 10 $headerBlue $lightGray $false
    SetC $ws4 $r 3 $r[1] $false 10 0 0 $true; MC $ws4 $r 3 8
    $ws4.Range("A$($r):H$($r)").RowHeight=28
    SB $ws4 $r 1 8; $r++
}

$ws4.Range("A:A").ColumnWidth=28
...
SetC $ws5 1 1 "降级方案  每周只做一件事  4周复盘  下一步" $true 16 $headerBlue 0 $true
...
# Slow habit path (V2 NEW)
$r+=2
SetC $ws5 $r 1 "  如果第一周同时做四件事太难——'每周只加一件事'路径" $true 14 $white $softPurple $true
MC $ws5 $r 1 8
$ws5.Range("A$($r):H$($r)").RowHeight=26
$ws5.Range("A$($r):H$($r)").Font.Color=$white
$ws5.Range("A$($r):H$($r)").Interior.Color=$softPurple
$r++

SetC $ws5 $r 1 "有些人的性格适合'一口吃下'，有些人需要'慢慢来'。如果第一周四件事让你焦虑——用这个版本。" $false 10 $headerBlue $lightGray $true
MC $ws5 $r 1 8; $r++

$slowHabits=@(
    @("第1周","只做一件事：戒含糖饮料","其他三件能做就做 做不到不批评自己。一周后'不喝甜饮料'已经不需要意志力了。"),
    @("第2周","戒含糖饮料 + 每天8000步","第一个习惯稳了 再加第二个。手机计步开着 碎片时间凑。"),
    @("第3周","戒含糖饮料 + 8000步 + 3L水","前两个已经不用'坚持'了——第三个很轻松。"),
    @("第4周","四件事全做到","这时候你已经是一个'有运动习惯的人'了。慢就是快。")
)
...
$progress=@(
    ...
    @("衣服","松了","裤腰能塞进一个手指 上衣不绷了 手臂线条出来了——体重秤测不出的变化。"),
