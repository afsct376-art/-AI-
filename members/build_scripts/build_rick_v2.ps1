...
SetC $ws0 1 1 "瑞克  力型兼备方案 V2" $true 22 $gold $darkBg $true
MC $ws0 1 1 5; $ws0.Range("A1:E1").RowHeight=46
$ws0.Range("A1:E1").Font.Color=$gold; $ws0.Range("A1:E1").Interior.Color=$darkBg

SetC $ws0 2 1 "崔知行教练  2026-06-18  |  168cm / 67kg  |  力型兼备  高翻高频技术  午间计时训练  V2=国家力量教练逻辑" $false 11 $white $darkBg $true
...
$terms=@(
    @("DUP","每日波动周期","每天练的不一样。周一重、周三重+技能、周五快+容量。不让身体适应同一种节奏。"),
    @("AMRAP","尽可能多做一次","最后一组不限次数 做到动作快变形了就停。用来测试你今天真实的极限。"),
    @("RPE","自感用力程度","做完一组凭感觉打分。1=躺着 10=拼老命。8-9=做完还剩1-2次力竭。"),
    @("高翻","Power Clean","举重动作——杠铃从地面翻到锁骨前。最难举重技术之一。【50%指高翻1RM的50% 没测过=流畅5次不砸锁骨的重量x0.5。】"),
    @("三倍伸展","Triple Extension","高翻的核心发力——髋、膝、踝三个关节同时伸展。不是用手臂拉 是用腿和髋爆发。"),
    @("超级组","Superset","两个动作连着做 中间不休息。A做完直接做B——压缩时间 增加代谢压力。"),
    @("速度日","Dynamic Effort","轻重量爆发力训练(50-65%1RM)。练神经驱动 不累积疲劳。【速度变慢=第3次明显比第1次慢 或需要硬挤。减慢减重5%。】")
)
foreach($t in $terms){
...
$row+=2
SetC $ws0 $row 1 "  V2 升级了什么——从'好计划'到'国家级力量教练逻辑'" $true 16 $white $softPurple $true
MC $ws0 $row 1 5; $ws0.Range("A$($row):E$($row)").RowHeight=32
$ws0.Range("A$($row):E$($row)").Font.Color=$white; $ws0.Range("A$($row):E$($row)").Interior.Color=$softPurple
$row++

$upgrades=@(
    @("  高翻 每周1次→每次训练前轻技术","放在每次训练热身之后 主项之前。5-8分钟 5x3@50-60%——高频低强度练神经适应。不影响后面主项。"),
    @("  周一卧推 4x5→3x5+1xAMRAP","前3组保证质量 第4组探底。AMRAP次数=每周进步的标尺。更安全(少一组疲劳) 更有效(知道真实极限)。"),
    @("  周五 纯容量→速度日+容量混合","主项速度卧推8x3@60-65%爆发力——练神经驱动 不累积疲劳。辅助做容量——补同一肌群。"),
    @("  深蹲 固定→4周轮换变式","第1-4周=高杠 第5-8周=低杠 第9-12周=前蹲 第13-16周=箱式。每4周换一次 避免适应性停滞。"),
    @("  午间策略 加精确组间计时","主项120s 辅助90s 孤立45-60s 超级组60s。手机计时器严格到秒——45min内完成所有内容。"),
    @("  减载周 换动作 不是做轻一点","同样动作做轻量=关节应力路径相同。换动作=给关节不同应力分布 真正的休息。"),
    @("  营养 加力型兼备热量决策标准","体重每周涨0.25-0.5kg=热量合适。涨超0.5kg=减200kcal。不动=加200kcal。力量连续2周不涨=查热量和睡眠。")
)
...
$row+=2
SetC $ws0 $row 1 "  DUP 3天分化——V2版 每次训练前都有高翻技术练习" $true 14 $white $softBlue $true
MC $ws0 $row 1 5; $ws0.Range("A$($row):E$($row)").RowHeight=26
...
SetC $ws0 $row 3 "V2核心方法" $true 10 $white $headerBlue $false
SetC $ws0 $row 4 "RPE" $true 10 $white $headerBlue $false
SetC $ws0 $row 5 "训练感觉" $true 10 $white $headerBlue $false; $row++

$dup=@(
    @("周一","上体力(大重量)","卧推3x5+1AMRAP 引体 推举 划船","8-9","前3组标准 第4组探底——重而可控"),
    @("周三","下肢力+高翻(技能+力量)","高翻5x3技术+深蹲4x5+RDL+核心","7-8","先磨技术 再冲力量——高翻在精力最好时"),
    @("周五","全身速度+容量","速度卧推8x3@60-65%+上斜+下拉+前蹲+手臂","7-8(速度日不追求RPE)","爆发力推 轻快有力——不累积疲劳")
)
foreach($d in $dup){
...
$o4=@(
    @("1","积累","85% 3x5+AMRAP 建基准","高翻50%技术 深蹲80%","速度60% 建基准 容量70%"),
    @("2","强化","+2.5-5kg AMRAP超第1周","高翻55% 深蹲+5kg","速度65% 容量缩短休息或+1组"),
    @("3","峰值","再+2.5-5kg 3x3+AMRAP","高翻60% 深蹲87.5% 4x3","速度65%挑战更快速 容量再缩短休息"),
    @("4","减载","60% 3x5 换动作","高翻50%练速度 深蹲60%","50%轻量 换俯卧撑/高脚杯/弹力带")
)
foreach($oi in $o4){
...
# 午间计时 V2
$row+=2
SetC $ws0 $row 1 "  午间训练——V2精确组间计时 45分钟必须完成" $true 14 $white $accentOrange $true
MC $ws0 $row 1 5; $ws0.Range("A$($row):E$($row)").RowHeight=26
...
$timer=@(
    @("热身(含高翻技术)","10min","划船机3min+动态拉伸2min+神经激活1min+高翻技术5x3@50-60% 4min"),
    @("主项","12min","卧推/深蹲 4组x120s=8min+前面热身组4min"),
    @("辅助项","13min","3个辅助动作 各3-4组x90s 加切换时间"),
    @("孤立项+拉伸","10min","手臂超级组/侧平举/核心+拉伸5min"),
    @("总时长","45min+拉伸5min=50min","午休一小时内完成 下午上班不迟到")
)
foreach($tm in $timer){
...
$idx=@(
    @("完整方案逻辑+DUP框架+V2升级详解"," Sheet 方案概览"),
    @("上体力 下肢力+高翻 速度+容量"," Sheet 训练计划"),
    @("渐进决策树+高翻进阶+深蹲变式轮换"," Sheet 渐进决策树"),
    @("午间热身+主动恢复+减载换动作"," Sheet 恢复策略"),
    @("力型兼备营养+热量决策+判断规则"," Sheet 饮食与规则"),
    @("精简版+力型兼备复盘+自主调节"," Sheet 备用与复盘")
)
foreach($ix in $idx){
...
SetC $ws1 1 1 "瑞克  力型兼备  方案概览 V2" $true 18 $headerBlue 0 $true
MC $ws1 1 1 9; $ws1.Range("A1:I1").RowHeight=36

SetC $ws1 2 1 "男 31-35岁 168cm/67kg 体脂20-25% 3-4年经验 力型兼备 高翻高频技术 午间计时训练 V2" $false 11 $gold $darkBg $true
...
$ov=@(
    @("训练频率","3次/周 DUP波动 每次含高翻技术练习(5-8min)","上体力(重)+下肢力+高翻(技能)+速度+容量(爆发+增肌)"),
    @("核心升级","高翻每练必做 卧推AMRAP 速度日 深蹲变式轮换 精确计时 减载换动作 热量决策","7大升级从'好计划'到'国家力量教练逻辑'"),
    @("周期结构","4周一循环(积累→强化→峰值→减载) 深蹲每4周轮换变式","每3个月全休1周"),
    @("午间策略","练前轻食+高翻技术10min+主项12min+辅助13min+孤立拉伸10min=45+5min","手机计时器严格到秒"),
    @("方案特点","力型兼备 DUP波动 高翻高频 速度日加持 午间精确计时 自主调节框架")
)
$r=5
foreach($o in $ov){
...
$r++
SetC $ws1 $r 1 "V2 七大升级详解" $true 14 $headerBlue $lightBlue $true; MC $ws1 $r 1 9; $r++

$v2why=@(
    "1. 高翻每次训练前都做（5-8min 5x3@50-60%）。高翻是技能——每周1次不够 每周3次轻技术比每周1次重技术进步更快。放在热身之后主项之前 神经系统最清醒。轻重量不累积疲劳 不影响后面主项。",
    "2. 卧推3x5+1AMRAP。前3组保证质量 第4组探底——比4x5更安全(少一组疲劳累积) 更有效(知道今天真实的次数极限)。AMRAP次数是每周进步的标尺。",
    "3. 周五速度日+容量混合。速度卧推8x3@60-65%爆发力——练神经驱动 不累积疲劳 周末能恢复。辅助做容量——上斜卧推+下拉+前蹲 补同一肌群但不同角度。",
    "4. 深蹲每4周轮换变式。高杠→低杠→前蹲→箱式。同一个变式做8周以上神经系统完全适应 进步变慢。前蹲还能直接帮高翻的接杠姿势。",
...
$rpeRef=@(
    @("7-8","中等偏强——做完还能做3-4次","周五速度+容量——泵感和爆发力"),
    @("8-9","很重——做完还能做1-2次","周一上体力AMRAP——探到真实极限"),
    @("技术(不评RPE)","不是力竭——是动作质量","高翻技术——磨动作 不追求RPE"),
    @("10","极限力竭","第3周峰值周AMRAP最后一组的最后1次")
)
...
$pr=@(
    @("1","高翻是技能 高频低强度学习","每次训练前5-8分钟磨技术。不是练到累——是练到准。技术对了重量自然来。"),
    @("2","力型兼备 两个维度一起走","周一冲大重量(力) 周五做速度+容量(型)。同一个人 两种刺激 两个维度都在涨。"),
    @("3","午间效率=质量不妥协","45分钟严格计时——但热身(含高翻)不跳过 动作质量不降。午休不等人。")
)
foreach($p in $pr){
...
SetC $ws2 1 1 "训练计划 V2——DUP 3天 每次含高翻技术 速度日+AMRAP" $true 16 $headerBlue 0 $true
...
# 周一 V2
$r=4
SetC $ws2 $r 1 "周一  上体力（大重量 3x5+1AMRAP）" $true 13 $white $darkBg $true
MC $ws2 $r 1 9; $ws2.Range("A$($r):I$($r)").RowHeight=28
$ws2.Range("A$($r):I$($r)").Font.Color=$white; $ws2.Range("A$($r):I$($r)").Interior.Color=$darkBg
$r++

SetC $ws2 $r 1 "先练高翻技术5x3@50%(4min)  主项卧推3x5+1AMRAP  辅助90s  孤立45-60s  组间严格计时。" $false 9 $headerBlue $lightGray $true
...
SetC $ws2 $r 7 "计时" $true 10 $white $headerBlue $false
SetC $ws2 $r 8 "力/型" $true 10 $white $headerBlue $false
SetC $ws2 $r 9 "技术要点" $true 10 $white $headerBlue $true
$ws2.Range("A$($r):I$($r)").RowHeight=26; $r++

$actsMon=@(
    @("高翻技术(先做!)","5组","3次","50%极限 RPE=技术不评","60s","4min","力+技能","【要点】髋爆发+耸肩+展体。不是用手臂拉——用髋和三倍伸展。轻重量练速度。"),
    @("杠铃卧推(主项)","3组+1AMRAP","5次","85%1RM RPE8-9 第4组AMRAP","120s","8min","力的标尺","【要点】起桥 肩胛收紧 瓦式。前3组标准5次 第4组做到技术变形前停。记录AMRAP次数。"),
    @("负重引体向上","3组","5-8次","自重+负重 RPE8","120s","6min","型的宽度","【要点】全幅度——底部伸直 顶部下巴过杠。做不了8次=减负重 能做8+=加负重。"),
    @("站姿杠铃推举","3组","6-8次","RPE7-8","90s","5min","力的辅助","【要点】核心收紧 不塌腰。杠铃从锁骨前推至头顶 不锁肘。"),
    @("杠铃俯身划船","3组","8-10次","RPE7-8","90s","5min","型的厚度","【要点】俯身45-60度 杠铃贴大腿前侧拉。引体=垂直拉 划船=水平拉 全角度。"),
    @("绳索下压+哑铃弯举超级组","各3组","各12-15次","轻-中 RPE7","组内不休息 组间60s","5min","型的手臂","【要点】下压做完直接弯举→休息60秒。大臂固定不借力。")
)
$i=1
foreach($a in $actsMon){
    ...
    SetC $ws2 $r 7 $a[6] $false 9 0 0 $false
    SetC $ws2 $r 8 $a[7] $false 9 0 0 $false
    SetC $ws2 $r 9 $a[8] $false 9 0 0 $true
    $ws2.Range("A$($r):I$($r)").RowHeight=50; SB $ws2 $r 1 9; $r++; $i++
}
$r+=2

# 周三 V2
SetC $ws2 $r 1 "周三  下肢力+高翻（技能+力量）" $true 13 $white $darkBg $true
MC $ws2 $r 1 9; $ws2.Range("A$($r):I$($r)").RowHeight=28
$ws2.Range("A$($r):I$($r)").Font.Color=$white; $ws2.Range("A$($r):I$($r)").Interior.Color=$darkBg
$r++

SetC $ws2 $r 1 "高翻技术5x3@50-55%(5min)  深蹲4x5@80%120s  辅助90s  孤立45s  组间严格计时。" $false 9 $headerBlue $lightGray $true
MC $ws2 $r 1 9; $ws2.Range("A$($r):I$($r)").RowHeight=20; $r++
...
$actsWed=@(
    @("高翻技术(先做! 比周一稍重)","5组","3次","50-55%极限 技术优先","60-90s","5min","力+技能","【要点】比周一稍重 但技术仍然是第一位。接杠要稳 杠铃轨迹要直。"),
    @("杠铃深蹲(主项 高杠)","4组","5次","80%1RM RPE7-8","120s","8min","力的核心","【要点】深度平行地面 膝盖不内扣。当前周期=高杠 躯干更竖直 股四主导。"),
    @("杠铃罗马尼亚硬拉","3组","8-10次","RPE7-8","90s","5min","型的后侧","【要点】膝盖微屈 全程直背。腘绳肌+臀=下肢后侧线条。和深蹲互补。"),
    @("悬垂举腿","3组","12-15次","自重","45s","3min","核心地基","【要点】下腹带动骨盆上卷 不靠惯性。高翻和深蹲都需要核心稳定。"),
    @("站姿提踵","3组","15-20次","RPE7","30s","2min","小腿收尾","【要点】顶峰停1秒 慢放。")
)
$i=1
foreach($a in $actsWed){
    SetC $ws2 $r 1 $i $false 10 0 $lightGray $false
    SetC $ws2 $r 2 $a[1] $true 10 $headerBlue 0 $false
    SetC $ws2 $r 3 $a[2] $false 10 0 0 $false
    SetC $ws2 $r 4 $a[3] $false 10 0 0 $false
...
# 周五 V2 速度+容量
SetC $ws2 $r 1 "周五  全身速度+容量（Dynamic Effort + Hypertrophy）" $true 13 $white $darkBg $true
MC $ws2 $r 1 9; $ws2.Range("A$($r):I$($r)").RowHeight=28
$ws2.Range("A$($r):I$($r)").Font.Color=$white; $ws2.Range("A$($r):I$($r)").Interior.Color=$darkBg
$r++

SetC $ws2 $r 1 "高翻技术5x3@50%(4min)  速度卧推8x3@60-65%60s  容量辅助90s  孤立45s  组间严格计时。" $false 9 $headerBlue $lightGray $true
MC $ws2 $r 1 9; $ws2.Range("A$($r):I$($r)").RowHeight=20; $r++
...
$actsFri=@(
    @("高翻技术(先做!)","5组","3次","50%极限 练速度","60s","4min","力+技能","【要点】轻重量 追求杠铃速度快。和三倍伸展的爆发力感觉。"),
    @("速度杠铃卧推(主项)","8组","3次","60-65%1RM 爆发力推","60s","8min","力的神经驱动","【要点】每组3次 爆发推起 控制下放。不累积疲劳——练神经 不是练肌肉。速度变慢=太重了 减重。"),
    @("上斜哑铃卧推","3组","10-12次","RPE7-8","90s","5min","型的上胸","【要点】凳角30-45度。和速度卧推互补——速度练神经 上斜补容量。"),
    @("高位下拉(宽握)","3组","12-15次","RPE7","60s","4min","型的宽度","【要点】宽握 杆拉至锁骨。和周一引体互补——轻量高次数 背阔肌外沿。"),
    @("杠铃前蹲","3组","8-10次","RPE7-8","90s","5min","力+型+高翻辅助","【要点】杠铃架在锁骨前 躯干更竖直。股四头主导。同时强化高翻接杠姿势。"),
    @("哑铃侧平举+弯举超级组","各3组","15次+12次","轻-中 RPE7","组内不休息 组间45s","4min","型的手臂+肩宽","【要点】侧平举做完直接弯举 休息45秒。短间歇=泵感拉满。")
)
$i=1
foreach($a in $actsFri){
    SetC $ws2 $r 1 $i $false 10 0 $lightGray $false
    SetC $ws2 $r 2 $a[1] $true 10 $headerBlue 0 $false
    SetC $ws2 $r 3 $a[2] $false 10 0 0 $false
    SetC $ws2 $r 4 $a[3] $false 10 0 0 $false
...
SetC $ws3 2 1 "训练像做实验——每次都是数据点。V2新增：深蹲每4周轮换变式 高翻技术判断树。" $false 10 $headerBlue $lightGray $true
MC $ws3 2 1 9
...
$progV2=@(
    @("1","50% 5x3 技术","85% 3x5+AMRAP 建基准","80% 4x5 高杠","速度60% 8x3 容量70%"),
    @("2","55% 5x3 稍重","+2.5-5kg AMRAP超第1周","+5kg","速度65% 容量缩短休息或+1组"),
    @("3","60% 4x3 挑战","再+2.5-5kg 3x3+AMRAP","87.5% 4x3","速度65%挑战更快速 容量再缩短"),
    @("4","50% 3x3 练速度","60% 3x5 换俯卧撑","60% 3x5 高脚杯","50%轻量 换弹力带")
)
foreach($pw in $progV2){
...
$squatCycle=@(
    @("第1-4周(本周期)","高杠深蹲","躯干更竖直 股四头肌主导","你现在用的——已适应 正是最佳期","和接杠姿势相似——直接帮助高翻"),
    @("第5-8周","低杠深蹲","躯干更前倾 后侧链主导","新刺激——前2周适应 后2周冲","强化后侧链——高翻启动更有力"),
    @("第9-12周","前蹲","杠铃架锁骨前 核心要求最高","最接近高翻接杠姿势——你的弱项","直接强化高翻接杠——最值得练"),
    @("第13-16周","箱式深蹲","坐箱起 练底部爆发力","新刺激——突破粘滞点","练底部爆发力——高翻的启动阶段")
)
foreach($sc in $squatCycle){
...
$cleanTreeV2=@(
    @("动作流畅 杠铃轨迹直 接杠稳","技术对了——可以尝试+2.5kg","但只在周三这天稍加 周一和周五保持轻量练速度"),
    @("手臂过早发力弯肘 用二头拉","技术不对——三倍伸展没出来","减重练髋爆发+耸肩。找'腿发力 手只是引导'的感觉"),
    @("接杠时杠铃砸锁骨 手腕疼","接杠位置不对 前架不稳","练前蹲+杠铃耸肩 找锁骨架杠的感觉。不加重。"),
    @("连续6次训练(2周) 技术流畅无卡顿","可以进入第2周——所有训练日都稍加重","周一50%→55% 周三55%→60% 周五50%→55%"),
    @("今天状态差/下午累了/午休不够","不练高翻——改练杠铃耸肩+前蹲轻量","高翻是最高神经需求——状态不好练了反而退步")
)
foreach($ct in $cleanTreeV2){
...
$treeAllV2=@(
    @("周一 3x5轻松+AMRAP比上周+2次","✅变强了","下次+2.5-5kg"),
    @("周一 AMRAP次数比上周少","⬇️累积疲劳或状态差","维持重量 不加重——不退步"),
    @("周三 高翻+深蹲后还有余力","✅可以加重","下周深蹲+2.5-5kg"),
    @("周五 速度卧推速度变慢","⚠️太重了","减重5%——速度优先于重量"),
    @("周五 容量部分休息觉得太长","⚠️代谢压力不够","缩短10-15秒"),
    @("某动作连续4周没进步","🔄需要换刺激","换变式——同肌群不同角度"),
    @("连续2周睡不好+训练状态差","该减载了","提前进入第4周减载")
)
foreach($ta in $treeAllV2){
...
$wuV2=@(
    ...
    @("高翻技术(每次必做!)","5x3@50-60% 轻重量练三倍伸展+接杠","4min")
)
foreach($w in $wuV2){
...
$deloadV2=@(
    @("高翻(重)","杠铃耸肩+前蹲(轻量 练分解)","2组x8-10"),
    @("杠铃卧推","俯卧撑(3秒下+爆发推)","3组x力竭60%"),
    @("杠铃深蹲","高脚杯深蹲(轻哑铃)","2组x10-12"),
    @("杠铃划船","哑铃单臂划船(轻量)","2组x10/侧"),
    @("负重引体","高位下拉(轻量)","2组x10-12"),
    @("有氧","低强度散步/休闲骑","20-30min <120bpm")
)
foreach($dl in $deloadV2){
...
$periV2=@(
    @("练前 11:30","香蕉+蛋白粉/全麦吐司2片+2蛋","轻食不撑胃 提供训练能量"),
    @("练后 13:00前","米饭+鸡胸肉/蛋白粉2勺+香蕉+牛奶","下午上班前关恢复窗口"),
    @("下午 15:00","坚果/水果/酸奶(饿了吃)","不饿可以不吃"),
    @("晚餐","正常吃：碳水一拳+蛋白质+蔬菜","和平时一样")
)
foreach($pw in $periV2){
...
# 热量决策 V2
$r+=2
SetC $ws5 $r 1 "力型兼备——热量调整决策标准 V2新增" $true 14 $white $darkBg $true
MC $ws5 $r 1 9; $ws5.Range("A$($r):I$($r)").RowHeight=26
...
$calDecisions=@(
    @("体重趋势","每周涨0.25-0.5kg","✅热量合适","继续——不调整"),
    @("体重趋势","每周涨0.5kg以上","⚠️热量偏高 可能涨脂肪","减200kcal(碳水减半碗饭)"),
    @("体重趋势","体重不动","⚠️热量偏低","加200kcal(碳水加半碗饭)"),
    @("体重趋势","体重下降","❌热量不够","加300kcal(碳水加一碗饭)"),
    @("力量趋势","卧推/深蹲每周能+2.5-5kg","✅热量合适","继续"),
    @("力量趋势","力量连续2周不涨","⚠️可能热量或睡眠不足","先查睡眠 再加碳水"),
    @("体脂趋势","腹肌线条还在","✅热量合适","继续"),
    @("体脂趋势","腹肌线条模糊","⚠️热量偏高","减200kcal"),
    @("体脂趋势","更清晰但力量不涨","⚠️热量偏低","加200kcal")
)
foreach($cd in $calDecisions){
...
$rulesV2=@(
    @("AMRAP次数比上周+2次以上","✅变强了——下次+2.5-5kg"),
    @("AMRAP次数没变或下降","➡️维持重量 不退步"),
    @("高翻连续6次训练流畅","+2.5kg——但只在周三这天加"),
    @("速度卧推时速度变慢","减重5%——速度优先于重量"),
    @("午间训练没力气","查11:30那顿——是不是没吃够"),
    @("中午练完下午犯困","练后碳水减半碗饭 蛋白不变"),
    @("状态极差","降级：热身+高翻技术+主项3组轻量+拉伸")
)
foreach($rv in $rulesV2){
...
$simpleV2=@(
    @("2练","上体力(精简)：高翻+卧推3x5+1AMRAP+引体+推举+划船+手臂","下肢力+全身(精简)：高翻+深蹲4x5+RDL+前蹲3x10+下拉+侧平举"),
    @("原则","高翻不跳——每练必做。保留核心复合 压缩辅助。","")
)
foreach($so in $simpleV2){
...
$reviewV2=@(
    @("力-卧推AMRAP","第1周做了几次→第3周做了几次？","涨=加重。平/降=维持或换变式"),
    @("力-深蹲4x5","重量涨了多少？","涨=加重。没涨=查恢复"),
    @("力-高翻技术","流畅了吗？能+2.5kg吗？","流畅=继续。仍卡=继续磨"),
    @("型-体重","涨0.5-1kg净肌肉？","涨=继续。没涨=加碳水"),
    @("型-照片","第1天vs第28天","镜子比秤真实"),
    @("力型兼备","力量涨+体型变=正在发生","继续——这套框架可以一直用")
)
foreach($rv in $reviewV2){
...
# ============================================
# SHEET 7: V3新增——第一周训练示例+每周检查清单+FAQ
# ============================================
...
$monExample=@(
    @("1","热身","划船机3min 拉伸2min 开合跳1min","—","—"),
    @("2","高翻技术","5组x3次@50%1RM(比如你能翻40kg→用20kg)","60s","轻快——磨三倍伸展"),
    @("3","杠铃卧推","40kgx5次x3组+第4组做到力竭(记次数!)","120s","重而可控"),
    @("4","负重引体","自重+5kgx5次x3组","120s","能做8次=下次加重"),
    @("5","杠铃推举","25kgx6次x3组","90s","核心收紧"),
    @("6","杠铃划船","30kgx8次x3组","90s","俯身45度 肩胛夹紧"),
    @("7","下压+弯举超级组","各15次x3组 做完下压直接弯举休息60s","组内不休息","手臂泵感"),
    @("8","拉伸","胸+背+肩+髋屈肌+腘绳肌 各30s","—","—")
)
foreach($me in $monExample){
...
$wedExample=@(
    @("1","热身","划船3min+拉伸2min+神经激活1min","—","—"),
    @("2","高翻技术","5组x3次@50-55%(稍重 比如25kg)","60-90s","技术仍第一位——接杠要稳"),
    @("3","杠铃深蹲(高杠)","50kgx5次x4组","120s","深度平行 膝盖不内扣"),
    @("4","杠铃RDL","35kgx8次x3组","90s","全程直背 腘绳肌拉伸"),
    @("5","悬垂举腿","15次x3组","45s","下腹发力 不靠惯性"),
    @("6","站姿提踵+拉伸","提踵20次x3组30s休息+拉伸5min","—","小腿收尾")
)
foreach($we in $wedExample){
...
$friExample=@(
    @("1","热身+高翻","高翻5组x3次@50%20kg 4min","60s","轻量练速度"),
    @("2","速度杠铃卧推","25kgx3次x8组(60-65%1RM)","60s","爆发推起 轻快——速度变慢=太重了"),
    @("3","上斜哑铃卧推","单边12kgx10次x3组","90s","上胸+肩前束"),
    @("4","高位下拉(宽握)","插片选10-12RMx12次x3组","60s","宽握 背阔外沿"),
    @("5","杠铃前蹲","30kgx8次x3组","90s","股四头主导 帮高翻接杠"),
    @("6","侧平举+弯举超级组","侧平举15次+弯举12次x3组 组内不休息","组间45s","手臂+肩收尾"),
    @("7","拉伸","全身拉伸5min","—","—")
)
foreach($fe in $friExample){
...
$checklist=@(
    @("本周体重涨了还是跌了？","___kg","___kg","___kg","___kg"),
    @("周一卧推AMRAP比上周多了几次？","___次","___次","___次","___次"),
    @("周三深蹲重量涨了吗？","___kg","___kg","___kg","___kg"),
    @("高翻技术流畅吗？有没有砸锁骨？","是/否","是/否","是/否","是/否"),
    @("本周睡眠平均几小时？","___h","___h","___h","___h"),
    @("本周哪次训练感觉特别好/特别差？","—","—","—","—")
)
...
$faqList=@(
    @("Q1:周一卧推AMRAP做到第几次算好？","第1周能多做2-3次就算好。比如3x5做完 第4组做了7次 那下次可以+2.5kg。"),
    @("Q2:周三深蹲4x5 最后一组做不完怎么办？","做不完就做3次 记录'4x5(3)'。下次维持重量 不加重。连续2周能做完 再+2.5kg。"),
    @("Q3:周五速度卧推8x3 做到第6组没力了？","减重5% 继续做完。速度日的核心是'完成8组' 不是'用多重'。"),
    @("Q4:中午练完下午犯困怎么办？","练后碳水减半碗。如果还困 把咖啡从练前移到练后喝。"),
    @("Q5:高翻总是砸锁骨怎么办？","减重 练前蹲+耸肩。高翻是技能不是力量——砸锁骨说明你接杠时机不对。"),
    @("Q6:如果不知道85%1RM是多少？","用你能做5次 但第5次很吃力的重量。那个重量大概就是你的85%1RM。"),
    @("Q7:减载周做完感觉'根本没练到'——正常吗？","完全正常。减载周就该轻松——不轻松说明你前三周没练到位。")
)
foreach($fq in $faqList){
...
# Save
$savePath="D:\Codex\members\瑞克\瑞克_力型兼备方案_V3.xlsx"
